...
public class DifferentListeners {
	...
	public static void main(String args[]) {
		...
		if (args.length == 0) {
			System.out
					.println("Usage :java DifferentSearches [EventContext |EventDirContext]");
			System.exit(0);
		}
	...
	/**
	 * Method to show usage of an adding an NamespaceChangeListener and an
	 * EventContext
	 */
	public void doEvtListener() {
		...
		try {
			...
			ctx.unbind("cn=eventtestrenamed,ou=JavaObjects"); // If this
																// example is
																// being rerun
																// multiple
																// times
			...
			ctx.rename("cn=eventtest,ou=JavaObjects",
					"cn=eventtestrenamed,ou=JavaObjects");
			Thread.sleep(3000);
			ctx.close();
		} catch (Exception e) {
	...
	/**
	 * Method to show usage of an adding a NamespaceChangeListener and an
	 * EventDirContext Make sure that ADDSerailize.java is executed so that the
	 * search returns some resulst before running this example
	 */
	public void doEvtDirListener() {
		try {
			Context inictx = getDirContext();
			EventDirContext ctx = (EventDirContext) (inictx
					.lookup("o=myserver.com"));
